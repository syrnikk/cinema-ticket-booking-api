...
from app.models.repertoir import Repertoir
...
def check_screening_availability(mapper, connection, target):
    existing_screening = db.query(Screening).join(Repertoir).join(Movie).filter(
        Repertoir.cinema_id == target.repertoir.cinema_id,
